...
import fund from "~/middleware/fund";
...
<template>
  <div class="space-y-6">
    <!-- <UInput v-model="q" name="q" placeholder="Search fund name or type" icon="i-heroicons-magnifying-glass-20-solid"
        ...
        <UTable :columns="columns" :rows="filteredRows">
          <template #name-data="{ row, getRowData }">
            <div class="max-w-[300px]">
              <UButton variant="link" :to="`/fund/${row.id}`" color="black" class="px-0 whitespace-pre-line">
                {{ getRowData() }}
              </UButton>
            </div>
          </template>
          ...
          <template #currentNav-data="{ row, getRowData }">
            <div class="flex items-center gap-x-2">
              ...
              <UBadge size="xs" variant="soft" :color="row.navPercentageChange < 0 ? 'red' : 'green'"
          ...
          <template #tags-data="{ row }">
            <UBadge :color="row.isShariah ? 'emerald' : 'blue'" variant="solid" class="w-full justify-center px-3">
